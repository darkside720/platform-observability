...
locals {
  ...
  pipeline_parameters = {
    ...
    cert_body        = get_env("TF_VAR_DUMMY_CERT_BODY", "")
    cert_chain       = get_env("TF_VAR_DUMMY_CERT_CHAIN", "")
    cert_private_key = get_env("TF_VAR_DUMMY_CERT_PRIVATE_KEY", "")
  }
}
...
inputs = {
  ...
  cert_body        = local.pipeline_parameters.cert_body
  cert_chain       = local.pipeline_parameters.cert_chain
  cert_private_key = local.pipeline_parameters.cert_private_key
}
